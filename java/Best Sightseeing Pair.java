 ...
 */
 ...
 class Solution {
    public int maxScoreSightseeingPair(int[] values) {
        ...
        // plus its index (because for the first element, index is 0, so it's just the
        // value).
        int maxValueWithIndex = values[0];

        // Iterate over the array starting from the 1st index since we've already
        // considered the 0th index.
        for (int j = 1; j < values.length; ++j) {
            ...
            // This score is computed as the value of the current element plus its 'value'
            // score (values[j])
            ...
            // Update the maxValueWithIndex to be the maximum of the current
            // maxValueWithIndex and
