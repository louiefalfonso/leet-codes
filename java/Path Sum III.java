...
class TreeNode {
 ...
 class Solution {
    // A map to store the cumulative sum up to all the ancestors of a node and their
    // respective counts.
    private Map<Long, Integer> cumulativeSumCount = new HashMap<>();
    // The target sum to find in the path.
    private int targetSum;

    // Public function to call the private dfs function and initialize the
    // cumulativeSumCount.
    ...
    // A private function to perform the DFS traversal and find paths with sums
    // equal to targetSum.
    private int dfs(TreeNode node, long currentSum) {
        // Base case: If the current node is null, return 0 as there are no paths
        // through this node.
        if (node == null) {
            return 0;
        }
