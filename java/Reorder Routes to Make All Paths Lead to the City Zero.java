...
class Solution {
    // Helper class to represent a pair of values consisting of a node and a boolean
    // indicating the edge's direction
    ...
    // `n` represents the number of nodes, and `connections` represents the directed
    // edges.
    ...
    private int dfs(int currentNode, Map<Integer, List<Edge>> graph, boolean[] visited) {
        ...
        // Get the edges connected to the current node; if not present, get an empty
        // list
        ...
        for (Edge edge : edges) {
            ...
            if (!visited[nextNode]) { // If the next node has not been visited
                if (isDirected) {
                    // If the current edge is directed towards the next node, it needs to be
                    // reordered.
                    reorderCount++;
                }
