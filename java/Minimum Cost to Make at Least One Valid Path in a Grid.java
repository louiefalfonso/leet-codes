...
class Solution {
    public int minCost(int[][] grid) {
        ...
        while (!queue.isEmpty()) {
            ...
            for (int k = 1; k <= 4; ++k) {
                ...
                if (newX >= 0 && newX < numRows && newY >= 0 && newY < numCols) {
                    // If the current direction is the same as the arrow in this cell (no cost to
                    // move here).
                    if (grid[i][j] == k) {
                        // Add the new cell at the front of the queue to explore it sooner (as it's no
                        // cost).
                        queue.offerFirst(new int[] { newX, newY, cost });
                    } else {
                        // Otherwise, add the new cell at the end of the queue and increase the cost by
                        // 1.
        ...
        // If the queue is empty and we didn't reach the bottom-right cell, return -1 as
        // it's not possible.
        return -1;
    }
}