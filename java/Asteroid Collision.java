    ...
    public int[] asteroidCollision(int[] asteroids) {
        ...
        for (int asteroid : asteroids) {
            ...
            if (asteroid > 0) {
                ...
                // If the top of the stack is an asteroid of the same magnitude but moving in
                // the opposite direction, destroy both
                if (!stack.isEmpty() && stack.peekLast() == -asteroid) {
                    stack.pollLast();
                } else if (stack.isEmpty() || stack.peekLast() < 0) {
                    // If there are no asteroids on the stack or the top asteroid is moving to the
                    // left, push the current asteroid onto the stack
                    stack.offerLast(asteroid);
                }
            }
        }
