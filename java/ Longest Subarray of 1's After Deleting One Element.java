 ...
 */
 ...
 class Solution {
    ...
    // Function to find the length of the longest subarray consisting of 1s after
    // deleting exactly one element.
    public int longestSubarray(int[] nums) {
        int length = nums.length;

        // Arrays to store the count of consecutive 1s to the left and right of each
        // index in nums
        ...
        // Count consecutive 1s from right to left, starting from the second-to-last
        // element
        ...
        for (int i = 0; i < length; ++i) {
            // Compute the length of subarray by removing the current element, hence adding
            // left and right counts of 1s.
            // Since one element is always removed, the combined length of consecutive 1s
            // from left and right
            // should not be equal to the total length of the array (which implies no 0 was
            // in the array to begin with).
            maxSubarrayLength = Math.max(maxSubarrayLength, leftOnesCount[i] + rightOnesCount[i]);
        }

        // Reduce the length by 1 if the length of consecutive 1s equals the array
        // length, since we need to remove one element.
