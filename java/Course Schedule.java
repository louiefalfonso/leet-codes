    ...
    public boolean canFinish(int numCourses, int[][] prerequisites) {
        ...
        while (!queue.isEmpty()) {
            ...
            for (int neighbor : graph.get(course)) {
                // Decrement the in-degree of each neighbor, since we have processed one of
                // their prerequisites
                inDegrees[neighbor]--;
                if (inDegrees[neighbor] == 0) {
                    // If in-degree becomes 0, it means all prerequisites are met, so enqueue the
                    // course
        ...
        // If the number of processed courses equals the total number of courses, all
        // can be finished
