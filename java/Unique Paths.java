 ...
 */
 ...
 class Solution {
    public int uniquePaths(int m, int n) {
        ...
        // Initialize the bottom row with 1s since there's only one way to reach each
        // cell in the bottom row
        // when only moving right.
        Arrays.fill(pathCounts, 1);

        // Loop over each cell starting from the second row up to the top row (since the
        // bottom row is already filled).
        for (int row = 1; row < m; ++row) {
            // For each cell in a row, start from the second column since the first column
            // of any row
            // will only have one unique path (i.e., moving down from the cell above).
            for (int col = 1; col < n; ++col) {
                // The number of unique paths to the current cell is the sum of the unique paths
                // to the cell
